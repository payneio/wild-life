...
Transport is Postgres ``LISTEN/NOTIFY`` on one channel (``personal_api_events``).
Publishers:
...
from personal_api.db.session import engine

log = logging.getLogger("personal_api.stream")

CHANNEL = "personal_api_events"
_MAX_STREAMS = 32
_active_streams = 0
...
async def publish_event(kind: str, data: dict[str, Any] | None = None) -> None:
    ...
    async with engine.connect() as conn:
        await conn.execute(text("SELECT pg_notify(:chan, :payload)"),
...
async def stream(request: Request) -> EventSourceResponse:
    ...
    async def gen() -> AsyncGenerator[dict[str, str], None]:
        ...
        if _active_streams >= _MAX_STREAMS:
            yield {"event": "error", "data": '{"kind":"error","reason":"too_many_streams"}'}
            return
        _active_streams += 1
